<!-- src/routes/transport-planning/traffic-impact-assessment/+page.svelte -->
<script>
	...
	const features = [
		{
			icon: FileText,
			title: 'تقييم الأثر المروري الشامل',
			titleEn: 'Comprehensive Traffic Impact Assessment',
			description: 'إعداد تقارير مفصلة لتحليل تأثير المشاريع التطويرية على شبكة النقل المحلية وتقديم الحلول المناسبة',
			descriptionEn: 'Preparing detailed reports analyzing development project impacts on local transport networks with appropriate solutions',
			color: 'from-emerald-500 to-blue-600',
			image: '/images/features/traffic-assessment.jpg'
		},
		{
			icon: Calculator,
			title: 'نمذجة SIDRA المتقدمة',
			titleEn: 'Advanced SIDRA Modeling',
			description: 'استخدام برامج SIDRA المتطورة لتحليل التقاطعات وتحسين تدفق المرور وتقليل أوقات الانتظار',
			descriptionEn: 'Using advanced SIDRA software for intersection analysis, traffic flow optimization and wait time reduction',
			color: 'from-purple-500 to-emerald-600',
			image: '/images/features/sidra-modeling.jpg'
		},
		{
			icon: Route,
			title: 'تحليل شبكة النقل',
			titleEn: 'Transport Network Analysis',
			description: 'تحليل شامل لشبكة النقل المحلية وتأثير حركة المرور الجديدة على الطرق والتقاطعات الموجودة',
			descriptionEn: 'Comprehensive analysis of local transport networks and new traffic impact on existing roads and intersections',
			color: 'from-blue-500 to-purple-600',
			image: '/images/features/network-analysis.jpg'
		},
		{
			icon: Users,
			title: 'تقييم وسائل النقل المتعددة',
			titleEn: 'Multi-Modal Transport Assessment',
			description: 'تحليل تأثير المشروع على جميع وسائل النقل بما في ذلك المشاة والدراجات والنقل العام',
			descriptionEn: 'Analyzing project impact on all transport modes including pedestrians, cycling, and public transport',
			color: 'from-amber-500 to-emerald-600',
			image: '/images/features/multi-modal.jpg'
		},
		{
			icon: Settings,
			title: 'تطوير تدابير التخفيف',
			titleEn: 'Mitigation Measures Development',
			description: 'تصميم وتطوير تدابير التخفيف المناسبة لضمان عدم تأثير المشروع سلبياً على شبكة النقل',
			descriptionEn: 'Designing and developing appropriate mitigation measures ensuring project does not negatively impact transport network',
			color: 'from-green-500 to-blue-600',
			image: '/images/features/mitigation-measures.jpg'
		},
		{
			icon: Shield,
			title: 'الامتثال للسياسات المحلية',
			titleEn: 'Local Policy Compliance',
			description: 'ضمان امتثال التقييم للسياسات والاستراتيجيات المحلية للتخطيط والنقل',
			descriptionEn: 'Ensuring assessment compliance with local planning and transport policies and strategies',
	...
	const benefits = [
		{
			title: 'تقارير قوية ومختصرة',
			titleEn: 'Robust and Succinct Reports',
			description: 'نقدم تقارير مفصلة وواضحة تلبي متطلبات السلطات المحلية وتسهل عملية الموافقة على المشاريع',
			descriptionEn: 'We provide detailed and clear reports meeting local authority requirements and facilitating project approvals',
			image: '/images/benefits/robust-reports.jpg'
		},
		{
			title: 'تحليل البيانات الدقيق',
			titleEn: 'Accurate Data Analysis',
			description: 'جمع وتحليل البيانات المرورية بدقة عالية لضمان صحة النتائج والتوصيات',
			descriptionEn: 'Efficient data collection and robust analysis ensuring accurate results and recommendations',
			image: '/images/benefits/data-analysis.jpg'
		},
		{
			title: 'حلول مخصصة للمشاريع',
			titleEn: 'Customized Project Solutions',
			description: 'تطوير حلول مخصصة لكل مشروع تأخذ في الاعتبار الظروف المحلية والمتطلبات الخاصة',
			descriptionEn: 'Developing customized solutions for each project considering local conditions and specific requirements',
			image: '/images/benefits/custom-solutions.jpg'
		}
	];
	...
	const stats = [
		{ 
			value: '500+', 
			label: 'تقييم أثر مروري مكتمل', 
			labelEn: 'Completed Traffic Impact Assessments', 
			icon: FileText,
			color: 'from-emerald-500 to-blue-600'
		},
		{ 
...
<svelte:head>
	<title>{($locale || 'en') === 'ar' ? 'تقييم الأثر المروري' : 'Traffic Impact Assessment'} - {($locale || 'en') === 'ar' ? APP_CONFIG.name : APP_CONFIG.nameEn}</title>
	<meta name="description" content="{($locale || 'en') === 'ar' ? 'خدمات تقييم الأثر المروري المتخصصة مع تقارير قوية ومختصرة لدعم مشاريع التطوير' : 'Specialized Traffic Impact Assessment services providing robust and succinct transport documents for development projects'}" />
</svelte:head>
...
<section class="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-slate-50 via-white to-emerald-50 dark:from-slate-900 dark:via-slate-800 dark:to-emerald-900">
	...
	<div class="absolute inset-0">
		...
		<svg class="absolute inset-0 w-full h-full opacity-30 dark:opacity-20" viewBox="0 0 1200 800">
			<defs>
				<pattern id="trafficGrid" width="100" height="100" patternUnits="userSpaceOnUse">
			...
			<path d="M0,200 Q300,100 600,200 T1200,200" stroke="url(#trafficWave)" stroke-width="4" fill="none" opacity="0.6" class="traffic-line-1"/>
			<path d="M0,400 Q400,300 800,400 T1200,400" stroke="url(#trafficWave)" stroke-width="3" fill="none" opacity="0.5" class="traffic-line-2"/>
			<path d="M0,600 Q200,500 400,600 T1200,600" stroke="url(#trafficWave)" stroke-width="2" fill="none" opacity="0.4" class="traffic-line-3"/>
		</svg>
	...
	<div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
		<div class="text-center">
			...
			<div class={`inline-flex items-center px-6 py-4 bg-gradient-to-r from-emerald-100/80 to-blue-100/80 dark:from-emerald-900/30 dark:to-blue-900/30 backdrop-blur-xl text-emerald-700 dark:text-emerald-300 rounded-full border border-emerald-200/50 dark:border-emerald-700/50 text-sm font-bold mb-8 transition-all duration-1000 ${mounted ? 'animate-slide-up-bounce' : 'opacity-0'} shadow-lg hover:shadow-2xl hover:scale-105`}>
				<Car class="w-4 h-4 mr-3 rtl:ml-3 rtl:mr-0" />
				<span class="mr-2 rtl:ml-2 rtl:mr-0 text-lg">🚦</span>
				{($locale || 'en') === 'ar' ? 'تقييم الأثر المروري' : 'Traffic Impact Assessment'}
			</div>
			...
			<div class={`transition-all duration-1000 delay-200 ${mounted ? 'animate-slide-up-bounce' : 'opacity-0'}`}>
				<h1 class="text-5xl lg:text-6xl xl:text-7xl font-black leading-tight mb-8">
					<span class="block bg-gradient-to-r from-emerald-600 via-blue-600 to-purple-600 bg-clip-text text-transparent animate-gradient-flow bg-300%">
						{($locale || 'en') === 'ar' ? 'تقييم الأثر المروري' : 'Traffic Impact'}
					</span>
					<span class="block text-2xl lg:text-3xl xl:text-4xl font-black text-slate-700 dark:text-slate-300 mt-6 animate-fade-in-delayed">
						{($locale || 'en') === 'ar' ? 'والتحليل المروري' : 'Assessment'}
					</span>
				</h1>
			</div>
			...
			<div class={`transition-all duration-1000 delay-400 ${mounted ? 'animate-slide-up-bounce' : 'opacity-0'}`}>
				<p class="text-xl lg:text-2xl text-slate-600 dark:text-slate-300 leading-relaxed max-w-4xl mx-auto mb-8">
					{($locale || 'en') === 'ar' 
						? 'نقدم تقارير نقل قوية ومختصرة أساسية لنجاح مشاريعك التطويرية'
						: 'Providing robust and succinct transport documents key to your development project success'
					}
				</p>
	...
	<div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
		<div class="text-center mb-16">
			<h2 class="text-3xl lg:text-4xl font-black text-slate-900 dark:text-white mb-4">
				{($locale || 'en') === 'ar' ? 'خدمات تقييم الأثر المروري' : 'Traffic Impact Assessment Services'}
			</h2>
			<p class="text-xl text-slate-600 dark:text-slate-300 max-w-3xl mx-auto">
				{($locale || 'en') === 'ar'
					? 'حلول شاملة لتقييم تأثير مشاريع التطوير على شبكات النقل المحلية'
					: 'Comprehensive solutions for assessing development project impacts on local transport networks'
				}
			</p>
		</div>
...
<section class="py-24 bg-white dark:bg-slate-900 relative overflow-hidden">
	...
	<div class="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
		<h2 class="text-3xl lg:text-4xl font-black text-slate-900 dark:text-white mb-4">
			{($locale || 'en') === 'ar' ? 'هل تحتاج تقييم أثر مروري لمشروعك؟' : 'Need a Traffic Impact Assessment for Your Project?'}
		</h2>
		<p class="text-xl text-slate-600 dark:text-slate-300 mb-8 leading-relaxed">
			{($locale || 'en') === 'ar'
				? 'إذا كنت بحاجة إلى تقييم أثر مروري كجزء من طلب التخطيط أو مشروع أعمال صغيرة، يرجى الاتصال بنا للمساعدة'
				: 'If you require a traffic impact assessment as part of a planning submission or small works project, please contact us for assistance'
			}
		</p>
...
<style>
	...
	.traffic-line-1 {
		...
		animation: traffic-line-flow 4s linear infinite;
	}
	
	.traffic-line-2 {
		stroke-dasharray: 15 8;
		animation: traffic-line-flow 5s linear infinite reverse;
	}
	
	.traffic-line-3 {
		stroke-dasharray: 10 5;
		animation: traffic-line-flow 6s linear infinite;
	}
