    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class={`mb-12 transition-all duration-1000 ${mounted ? 'animate-fade-in-up' : 'opacity-0'}`}>
        <div class="aspect-w-16 aspect-h-9 rounded-2xl overflow-hidden shadow-2xl">
          <div class="w-full h-96 bg-gradient-to-br from-emerald-100 to-emerald-200 dark:from-emerald-900/30 dark:to-emerald-800/30 flex items-center justify-center">
            <div class="text-center">
      ...
      <div class={`flex items-center justify-between mb-12 p-6 bg-slate-50 dark:bg-slate-800 rounded-2xl transition-all duration-1000 delay-200 ${mounted ? 'animate-fade-in-up' : 'opacity-0'}`}>
        <div class="flex items-center gap-4">
          ...
          <button
            onclick={handleShare}
            class="flex items-center px-4 py-2 bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl font-medium hover:bg-emerald-100 dark:hover:bg-emerald-900/30 transition-colors"
          >
        ...
        <div class="flex flex-wrap gap-2">
          {#each (($locale || 'en') === 'ar' ? post.tags : post.tagsEn).slice(0, 3) as tag}
            <span class="inline-flex items-center px-3 py-1 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 text-sm rounded-lg">
      ...
      <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {#each relatedPosts as relatedPost, index}
          <article class={`bg-white dark:bg-slate-900 rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border border-slate-200 dark:border-slate-700 ${mounted ? 'animate-fade-in-up' : 'opacity-0'}`} style="animation-delay: {index * 0.2}s;">
            ...
            <!-- Image -->
            <div class="h-48 bg-gradient-to-br from-slate-200 to-slate-300 dark:from-slate-700 dark:to-slate-600 flex items-center justify-center">
              <div class="text-center">
                <div class="text-4xl mb-2">
                  {relatedPost.categoryEn === 'Highway Engineering' ? '🛣️' : 
                   relatedPost.categoryEn === 'Traffic Engineering' ? '🚦' : 
                   relatedPost.categoryEn === 'Transport Planning' ? '🗺️' : '📝'}
                </div>
            ...
            <div class="p-6">
              ...
              <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-3 leading-tight">
